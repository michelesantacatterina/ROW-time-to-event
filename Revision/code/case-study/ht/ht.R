...
sl.lib_ps  <- c('SL.glm',"SL.ranger",'SL.glm.FP','SL.glm.BS')

#Balancing covariates between treatment groups (binary)
Tr    <- abs(intervention_1-1)
timeIPW1    <- tryCatch(  system.time( SL1    <- SuperLearner(Tr, confounders_1 ,
                                                             SL.library = sl.lib_ps, 
                                                             family = "binomial"))
                          , error=function(e) NULL)#endtryCatch
ps1    <- SL1$SL.predict
ipw   <- ( Tr/ps1 + (1-Tr)/(1-ps1)  )
summary(ipw)

ipw1_S   <- ( Tr*(mean(Tr)/ps1) + (1-Tr)*(mean(1-Tr)/(1-ps1))  )
summary(ipw1_S)
...
ipw1_T[which(ipw1_T>=quantile(ipw1_T,QQ))] <- quantile(ipw1_T,c(QQ))
ipw1_T[which(ipw1_T<=quantile(ipw1_T,1-QQ))] <- quantile(ipw1_T,c(1-QQ))
...
timeIPW2    <- tryCatch(  system.time( SL2    <- SuperLearner(Tr, confounders_2 ,
                                                             SL.library = sl.lib_ps, 
                                                             family = "binomial"))
                          , error=function(e) NULL)#endtryCatch
ps2    <- SL2$SL.predict
ipw   <- ( Tr/ps2 + (1-Tr)/(1-ps2)  )
summary(ipw)

ipw2_S   <- ( Tr*(mean(Tr)/ps2) + (1-Tr)*(mean(1-Tr)/(1-ps2))  )
summary(ipw2_S)
...
ipw2_T[which(ipw2_T>=quantile(ipw2_T,QQ))] <- quantile(ipw2_T,c(QQ))
ipw2_T[which(ipw2_T<=quantile(ipw2_T,1-QQ))] <- quantile(ipw2_T,c(1-QQ))
...
timeIPW3    <- tryCatch(  system.time( SL3    <- SuperLearner(Tr, confounders_3 ,
                                                             SL.library = sl.lib_ps, 
                                                             family = "binomial"))
                          , error=function(e) NULL)#endtryCatch
ps3    <- SL3$SL.predict
ipw   <- ( Tr/ps3 + (1-Tr)/(1-ps3)  )
summary(ipw)

ipw3_S   <- ( Tr*(mean(Tr)/ps3) + (1-Tr)*(mean(1-Tr)/(1-ps3))  )
summary(ipw3_S)
...
ipw3_T[which(ipw3_T>=quantile(ipw3_T,QQ))] <- quantile(ipw3_T,c(QQ))
ipw3_T[which(ipw3_T<=quantile(ipw3_T,1-QQ))] <- quantile(ipw3_T,c(1-QQ))
...
n <- dim(confounders_1)[1]
am_ipw_1_S <- bal.tab(intervention_1~confounders_1,
                    weights=ipw1_S,
                    method="weighting",
                    s.d.denom="pooled")$Balance$Diff.Adj

surveyDesignipw_1  <- svydesign(ids=1:n,weights=~ipw1_S,data=data_imputed_1)
modelipw_1_S   <- svycoxph(Surv(TIME_CHD, failure) ~ totp,design=surveyDesignipw_1) 
summary(modelipw_1_S)



n <- dim(confounders_2)[1]
am_ipw_2_S <- bal.tab(intervention_2~confounders_2,
                      weights=ipw2_S,
                      method="weighting",
                      s.d.denom="pooled")$Balance$Diff.Adj

surveyDesignipw_2  <- svydesign(ids=1:n,weights=~ipw2_S,data=data_imputed_2)
modelipw_2_S   <- svycoxph(Surv(TIME_CHD, failure) ~ totp,design=surveyDesignipw_2) 
summary(modelipw_2_S)



n <- dim(confounders_3)[1]
am_ipw_3_S <- bal.tab(intervention_3~confounders_3,
                      weights=ipw3_S,
                      method="weighting",
                      s.d.denom="pooled")$Balance$Diff.Adj

surveyDesignipw_3  <- svydesign(ids=1:n,weights=~ipw3_S,data=data_imputed_3)
modelipw_3_S   <- svycoxph(Surv(TIME_CHD, failure) ~ totp,design=surveyDesignipw_3) 
summary(modelipw_3_S)
...
n <- dim(confounders_1)[1]
am_ipw_1_T <- bal.tab(intervention_1~confounders_1,
                      weights=ipw1_T,
                      method="weighting",
                      s.d.denom="pooled")$Balance$Diff.Adj

surveyDesignipw_1  <- svydesign(ids=1:n,weights=~ipw1_T,data=data_imputed_1)
modelipw_1_T   <- svycoxph(Surv(TIME_CHD, failure) ~ totp,design=surveyDesignipw_1) 
summary(modelipw_1_T)



n <- dim(confounders_2)[1]
am_ipw_2_T <- bal.tab(intervention_2~confounders_2,
                      weights=ipw2_T,
                      method="weighting",
                      s.d.denom="pooled")$Balance$Diff.Adj

surveyDesignipw_2  <- svydesign(ids=1:n,weights=~ipw2_T,data=data_imputed_2)
modelipw_2_T   <- svycoxph(Surv(TIME_CHD, failure) ~ totp,design=surveyDesignipw_2) 
summary(modelipw_2_T)



n <- dim(confounders_3)[1]
am_ipw_3_T <- bal.tab(intervention_3~confounders_3,
                      weights=ipw3_T,
                      method="weighting",
                      s.d.denom="pooled")$Balance$Diff.Adj

surveyDesignipw_3  <- svydesign(ids=1:n,weights=~ipw3_T,data=data_imputed_3)
modelipw_3_T   <- svycoxph(Surv(TIME_CHD, failure) ~ totp,design=surveyDesignipw_3) 
summary(modelipw_3_T)
