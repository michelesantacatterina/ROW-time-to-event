...
depCensor     <- FALSE
depTreat      <- TRUE
seq_rateC     <- c(0.1,1,10,100,1000)
seq_miss      <- seq(0,1,length.out=5)
seq_pos       <- seq(0.1,2,length.out=5)
seq_scenario  <- 1:5
...
Bias <- MSE <- Bal <- Time <- Pcens <- Pscen <-  NULL
SES <- SEN <- SEB <- SER <-  TimeR <- TimeN <- TimeB <- Co95B <- Co95R <- Co95N <- NULL

sl.lib_gps <- c('SL.glm','SL.glm.interaction',
                'SL.glmnet',  'SL.rpart',"SL.bayesglm",
                "SL.loess",'SL.xgboost')
sl.lib_ps  <- c('SL.glm','SL.glm.interaction',
                'SL.glmnet',  'SL.rpart',"SL.bayesglm",'SL.xgboost')
  ...
  for(type_treatment in seq_type_trt){
    if(type_treatment=="binary"){
      for(beta_pos in seq_pos){
        ...
        for(i in 1:itera){
          dta <- data_generation(n=n, lambda=lambda, rho=rho, delta = true_haz, 
...
###################################################################################################################################################
      ...
      seq_method_con <- c("IPW","BalSL","GBM","CBPS","npCBPS","OM","naive","ROW")
      seq_method_bin <- c("IPW","BalSL","GBM","PSM","CBPS","EBAL","SBW","OM","naive","ROW")
      seq_type_trt <- c("binary", "continuous")
      #
      # seq_method_con <- c("IPW","CBPS","OM","naive","ROW")
      ...
      name_bias <- c("Method", "Treatment", "Positivity", "Bias")
      name_mse  <- c("Method", "Treatment", "Positivity", "MSE")
      name_bal  <- c("Method", "Treatment", "Positivity", "Balance")
      name_time <- c("Method", "Treatment", "Positivity", "Time")
      
      rateCens  <- 0.1  #Minimal Censoring
      beta_miss  <- 0    #No model misspecification
      scenario <- 1 #for binary treatment
      for(type_treatment in seq_type_trt){
      ...
      save.image(paste("data_results/simu_n",n,"positivity.Rdata"))
      ...
      name_bias <- c("Method", "Treatment", "Misspecification", "Bias")
      name_mse  <- c("Method", "Treatment", "Misspecification", "MSE")
      ...
      save.image(paste("data_results/simu_n",n,"misspecified.Rdata"))
      ...
      save.image(paste("data_results/simu_n",n,"censoring.Rdata"))
...
###################################################################################################################################################
        ...
        for(method in seq_method){
          ...
          if(type_treatment=="binary"){
            ...
            for(beta_pos in seq_pos){
              j <- j+1
              print(paste(":::::::::::::::::::::::: Positivity",beta_pos, "- Treatment", type_treatment))
              res     <- compute_errors(itera,method,rateCens,type_treatment,beta_pos,beta_miss,n,scenario,itera_boot,true_haz_exp,delta)
              SES[j]  <- res$ses
              SEB[j]  <- res$seb
              SEN[j]  <- res$sen
              SER[j]  <- res$ser
            ...
            ########################################################################
            #Coverage
            temp_co           <- data.frame( c(rep('Bootstrap',length(seq_pos)),
                                               rep('Robust',length(seq_pos)),
                                               rep('Naive',length(seq_pos) ) 
                                              ),
                                              rep(type_treatment,length(seq_pos)),
      ...
      save.image(paste("data_results/simu_errors_n",n,"positivity.Rdata"))
      ...
      for(type_treatment in seq_type_trt){
        ...
        for(method in seq_method){
          print(paste("########################### Method --->", method))
            ...
            ########################################################################
            #Coverage
            temp_co           <- data.frame( c(rep('Bootstrap',length(seq_miss)),
                                               rep('Robust',length(seq_miss)),
                                               rep('Naive',length(seq_miss) ) 
            ),
            rep(type_treatment,length(seq_miss)),
      ...
      save.image(paste("data_results/simu_errors_n",n,"misspecification.Rdata"))
      ...
      save.image(paste("data_results/simu_errors_n",n,"censoring.Rdata"))
...
###################################################################################################################################################
      ...
      save.image(paste("data_results/simu_n_sample_size.Rdata"))
...
###################################################################################################################################################
      ...
      n         <- 1000
      seq_delta <- c(0.0001, 0.001, 0.01, 0.1, 1) 
      seq_delta <- c(0.0001, 0.025, 0.05, 0.075, 0.1) 
      ...
      save.image(paste("data_results/simu_n",n,"delta.Rdata"))
